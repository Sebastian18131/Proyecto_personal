...
class PQRController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // 🔹 Add filters (radicado, año, destinatario, tipo)
        if ($request->has('radicado')) {
            $query->where('radicado', 'LIKE', '%' . $request->query('radicado') . '%');
        }
        if ($request->has('year')) {
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'document_type' => 'required|string|in:CC,TI,CE,OTRO',
